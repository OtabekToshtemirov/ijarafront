...
export default function CustomerDetailsPage() {
  ...
  const rentals =
    useSelector((state) =>
      state.rentals.rentals.filter((r) => r.customer?._id === id)
    ) || [];
  ...
  const totalReturnsCost = rentals.reduce((sum, rental) => {
    return (
      sum +
      (rental.returnedProducts || []).reduce(
        (returnSum, returnedProduct) => returnSum + (returnedProduct.cost || 0),
        0
      )
    );
  }, 0);
  ...
  const getUnreturnedProducts = () => {
    return activeRentals.flatMap((rental) => {
      ...
      return rental.borrowedProducts
        .filter((borrowedProduct) => {
          // Qaytarilgan miqdorni hisoblaymiz
          const returnedQuantity =
            rental.returnedProducts
              ?.filter(
                (returnedProduct) =>
                  returnedProduct.product === borrowedProduct.product
              )
              ?.reduce(
                (sum, returnedProduct) => sum + returnedProduct.quantity,
                0
              ) || 0;
    ...
    return rentals.flatMap((rental) => {
      return (rental.returnedProducts || []).map((returnedProduct) => {
        ...
        return {
          ...
          dailyRate:
            returnedProduct.dailyRate || borrowedProduct?.dailyRate || 0,
          totalCost: returnedProduct.totalCost || 0,
          discountDays: returnedProduct.discountDays || 0,
          productName:
            product?.name ||
            borrowedProduct?.product?.name ||
            "Noma'lum mahsulot",
        };
      });
    });
  ...
  const getTotalDiscounts = () => {
    ...
    return customer.rentals.reduce((total, rental) => {
      ...
      const paymentDiscounts = (rental.payments || []).reduce(
        (sum, payment) => sum + (payment.discount || 0),
        0
      );

      // Get discounts from returned products
      const returnDiscounts = (rental.returnedProducts || []).reduce(
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <div className="grid gap-6">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-medium">Исм:</span> {customer.name}
                  </p>
                  <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-medium">Изоҳ:</span> {customer.description}
                  </p>
                  <p className="text-gray-700 dark:text-gray-300">
                  ...
                  <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-medium">Манзил:</span>{" "}
                    {customer.address}
                  </p>
                  Статус:<Badge
                    className={
                      customer.status === "VIP"
                        ? "bg-yellow-500 dark:bg-yellow-600 ml-2"
                        : "bg-blue-500 dark:bg-blue-600 ml-2"
                    }
                  >
                ...
                <div className="space-y-2">
                  <p className="text-gray-700 dark:text-gray-300">
                    ...
                    >
                      {totalPayments -
                        ...
                        payments.reduce(
                          (total, payment) => total + (payment.discount || 0),
                          0
                        )}{" "}
                      сўм
                    </span>
                  ...
                  <p className="text-gray-700 dark:text-gray-300">
                    ...
                    <span className="text-green-600 dark:text-green-400">
                      {" "}
                      {totalPayments.toLocaleString()} сўм
                    </span>
                  </p>
                  <p className="text-gray-700 dark:text-gray-300">
              ...
              <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-gray-700 dark:text-gray-300">
                    ...
                    <span className="text-purple-600 dark:text-purple-400">
                      {" "}
                      {getUnreturnedProducts().length} та
                    </span>
                  </p>
                </div>
              </div>
                    ...
                    {activeRentals.map((rental) => (
                      ...
                      >
                        <TableCell className="dark:text-gray-300">
                          {new Date(
                            rental.workStartDate || rental.createdAt
                          ).toLocaleDateString()}
                        </TableCell>
                        <TableCell className="dark:text-gray-300">
                          {rental.rentalNumber}
                        ...
                        <TableCell className="dark:text-gray-300">
                          {rental.totalCost?.toLocaleString()} сўм
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                  ...
                  <TableBody>
                    {getUnreturnedProducts().map((product, index) => (
                      <TableRow key={index} className="dark:border-gray-700">
                        ...
                        <TableCell className="dark:text-gray-300">
                          {new Date(
                            product.rental.workStartDate ||
                              product.rental.createdAt
                          ).toLocaleDateString()}
                        </TableCell>
                        <TableCell className="dark:text-gray-300">
                          {product.remainingQuantity} дона
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                    ...
                    {payments.map((payment) => (
                      ...
                      >
                        ...
                        <TableCell className="text-orange-600">
                          {payment.discount > 0
                            ? `${payment.discount?.toLocaleString()} сўм`
                            : "-"}
                        </TableCell>
                        <TableCell className="dark:text-gray-300">
                          {payment.description || "-"}
        ...
        <Card className="mt-8">
          <CardHeader>
            <CardTitle>Қайтариш тарихи</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {getReturnHistory().map((returnItem, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      Ижара № {returnItem.rental.rentalNumber}
                    </TableCell>
                    <TableCell>
                      {returnItem.product.name}
                      {returnItem.productCode && (
                        <div className="text-xs text-muted-foreground">
                          Код: {returnItem.productCode}
                        </div>
                      )}
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      {returnItem.discount > 0 && (
                        <div className="text-xs text-muted-foreground bg-yellow-400 text-black p-1 ml-2 inline rounded-lg">
                          Чегирма: {returnItem.discount} кун
                        </div>
                      )}
                      <div className="text-xs text-muted-foreground">
                        {new Date(returnItem.startDate).toLocaleDateString()} -{" "}
                        {new Date(returnItem.returnDate).toLocaleDateString()}
                      </div>
                    </TableCell>
                    <TableCell>
                      {returnItem.dailyRate?.toLocaleString()} сўм
                    </TableCell>
                    <TableCell>
                      {returnItem.totalCost?.toLocaleString()} сўм
                      <div className="text-xs text-muted-foreground">
                        {returnItem.dailyRate?.toLocaleString()} ×{" "}
                        {returnItem.days} кун × {returnItem.quantity} дона
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
                {!rentals.some(
                  (rental) => rental.returnedProducts?.length > 0
                ) && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center">
                      Қайтариш тарихи мавжуд эмас
                    </TableCell>
                  </TableRow>
                )}
                {rentals.some(
